...
public class VideoFolderAdapter extends RecyclerView.Adapter<VideoFolderAdapter.MyViewHolder> {
    ...
    public void onBindViewHolder(@NonNull MyViewHolder myViewHolder, int i) {
        myViewHolder.fileName.setText(folderVideoFiles.get(i).getTitle());
        String time = folderVideoFiles.get(i).getDuration();
        if(time != null){
        ...
        Glide.with(mContext)
                .load(new File(folderVideoFiles.get(i).getPath()))
            ...
            public void onClick(View view) {
                ...
                popupMenu.setOnMenuItemClickListener(new PopupMenu.OnMenuItemClickListener() {
                    ...
                    public boolean onMenuItemClick(MenuItem menuItem) {
                        ...
                        {
                            case R.id.delete:
                                File file = new File(folderVideoFiles.get(i).getPath());
                                final Uri trackuri= ContentUris.withAppendedId(
                                        android.provider.MediaStore.Video.Media.EXTERNAL_CONTENT_URI, Long.parseLong(folderVideoFiles.get(i).getId()));
                                mContext.getContentResolver().delete(trackuri, null,null);
                                folderVideoFiles.remove(i);
                                notifyItemRemoved(i);
                                notifyItemRangeChanged(i,folderVideoFiles.size());
                                if(file.exists()){
